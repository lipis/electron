...
**the main process**. The script that runs in the main process can display a GUI
...
`BrowserWindow` instance is destroyed, the corresponding renderer process is
also terminated.
...
[remote](../api/remote.md) module for RPC style communication. There is also an
FAQ entry on [how to share data between web pages][share-data].
...
The `main.js` should create windows and handle system events, a typical example
being:
...
files, you'll probably want to try running your app locally to test it and make
sure it's working as expected.

### `electron`

[`electron`](https://github.com/electron-userland/electron-prebuilt) is an `npm`
...
If you downloaded Electron manually, you can also use the included binary to
execute your app directly.
...
After you're done writing your app, you can create a distribution by following
the [Application Distribution](./application-distribution.md) guide and then
executing the packaged app.
...
[list of boilerplates](https://electronjs.org/community#boilerplates) created by
the awesome electron community.
