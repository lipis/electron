...
Open `chrome://components/` in Chrome browser, find `WidevineCdm` and make sure
it is up to date, then you can find all the plugin binaries from the
...
* Open devtools and check whether `navigator.plugins` includes the Widevine CDM
  plugin.
