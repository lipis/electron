...
[Coding Style](http://www.chromium.org/developers/coding-style). You can use
...
recommended to get acquainted with them. A good place to start is Chromium's
...
* File names should be concatenated with `-` instead of `_`, e.g. `file-name.js`
  rather than `file_name.js`, because in
  [github/atom](https://github.com/github/atom) module names are usually in the
  `module-name` form. This rule only applies to `.js` files.
...
jQuery's one-function style. For example, `.getText()` and `.setText(text)` are
preferred to `.text([text])`. There is a
