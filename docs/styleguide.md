...
Each page must use the actual object name returned by `require('electron')` as
the title, such as `BrowserWindow`, `autoUpdater`, and `session`.
...
* All methods that have a return value must start their description with
  ...
  * If the method returns an `Object`, its structure can be specified using a
    colon followed by a newline then an unordered list of properties in the same
    style as function parameters.
...
The title can be `###` or `####`-levels depending on whether it is a method of a
module or a class.
...
The optional arguments are notated by square brackets `[]` surrounding the
optional argument as well as the comma required if this optional argument
...
`Array` type arguments must specify what elements the array may include in the
description below.
...
The title can be `###` or `####`-levels depending on whether it is an event of a
module or a class.
